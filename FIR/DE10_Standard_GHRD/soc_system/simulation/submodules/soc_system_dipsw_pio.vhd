...
entity soc_system_dipsw_pio is 
        port (
              -- inputs:
...
end entity soc_system_dipsw_pio;


architecture europa of soc_system_dipsw_pio is
...
begin
  ...
  read_mux_out <= (((A_REP(to_std_logic((((std_logic_vector'("000000000000000000000000000000") & (address)) = std_logic_vector'("00000000000000000000000000000000")))), 10) AND data_in)) OR ((A_REP(to_std_logic((((std_logic_vector'("000000000000000000000000000000") & (address)) = std_logic_vector'("00000000000000000000000000000010")))), 10) AND irq_mask))) OR ((A_REP(to_std_logic((((std_logic_vector'("000000000000000000000000000000") & (address)) = std_logic_vector'("00000000000000000000000000000011")))), 10) AND edge_capture));
  process (clk, reset_n)
  begin
    if reset_n = '0' then
  ...
  begin
    if reset_n = '0' then
      ...
      if std_logic'(clk_en) = '1' then 
        if std_logic'((edge_capture_wr_strobe AND writedata(8))) = '1' then 
          edge_capture(8) <= std_logic'('0');
  ...
  begin
    if reset_n = '0' then
      ...
      if std_logic'(clk_en) = '1' then 
        if std_logic'((edge_capture_wr_strobe AND writedata(9))) = '1' then 
          edge_capture(9) <= std_logic'('0');
